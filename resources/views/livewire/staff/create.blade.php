...
<div wire:ignore.self class="modal fade" id="modal-add-staff-part" tabindex="-1" role="dialog" aria-labelledby="addStaffModalLabel" aria-hidden="true">
            ...
            <div class="modal-body">           
                <form class="">
                    ...
                    <div class="form-group">
                        ...
                        <select wire:model="department" class="form-control selectric">
                            <option value="">Select a department</option>
                            @foreach($departments as $department)
                            @if($department == $selected_department)
                            <option selected value="{{ $department->name }}">{{ ucfirst($department->name) }}</option>
                            @else
                            <option value="{{ $department->name }}">{{ ucfirst($department->name) }}</option>
                            @endif
                            @endforeach
                        </select>
                    </div>
